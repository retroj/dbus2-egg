...
(let ((connections '()) ;; an alist mapping bus to DBusConnection ptrs
  ...
  (set! make-context
    (lambda (#!key (bus session-bus) service interface (path "/"))
  ...
  (set! poll-for-message
    (lambda (#!key (bus session-bus) (timeout 0))
  ...
  (set! enable-polling-thread!
    (lambda (#!key (bus session-bus) (enable #t) (interval default-polling-interval))
